...
const Student = () => {
  ...
  // Audio referenslarni saqlash uchun ro'yxat
  const audioRefs = useRef([]);

  // Audio o'ynashni boshqarish funksiyasi
  const handleAudioPlay = (currentAudioRef) => {
    // Boshqa audio fayllarni to'xtatish
    audioRefs.current.forEach((ref) => {
      if (ref && ref !== currentAudioRef.current && !ref.paused) {
        ref.pause();
      }
    });
    // Joriy audio refni ro'yxatga qo'shish (agar hali qo'shilmagan bo'lsa)
    if (!audioRefs.current.includes(currentAudioRef.current)) {
      audioRefs.current.push(currentAudioRef.current);
    }
  };
                          ...
                          <div key={taskIndex}>
                            {task.files.map((file, fileIndex) => (
                              <li key={fileIndex}>
                                <FileRenderer
                                  file={file}
                                  onPlay={handleAudioPlay} // onPlay funksiyasini uzatish
                                />
                              </li>
                            ))}
                          </div>
